...
from company_ai.api.rate_limit import BUDGET_MESSAGE, check_daily_budget, record_token_usage, session_message_limiter
from company_ai.config import Settings
from company_ai.db import connect_dict
from company_ai.llm.citations import citation_label, insufficient_evidence_validation, render_structured_answer
from company_ai.llm.client import LlmClient, OpenAICompatibleClient
from company_ai.llm.prompts import ANSWER_RESPONSE_FORMAT, DEFAULT_CONTEXT_TOKEN_BUDGET, RESPONSE_MAX_TOKENS, build_answer_messages, render_context_prompt
from company_ai.llm.streaming import StructuredAnswerStreamer
from company_ai.models import ConversationMessage, ConversationThread
from company_ai.retrieval.intent import IntentResolver, IntentResult
from company_ai.retrieval.retriever import AccountResolutionError, PostgresHybridRetriever, _normalize_for_match
...
# account's namespace; DemoNote is session-scoped so the id has no account segment.
SOURCE_OBJECT_SPECS: dict[str, tuple[str, str, bool]] = {
    "Email": ("email", "Email", True),
    "PDF": ("pdf", "PDF", True),
    "WordDocument": ("docx", "Word", True),
    "Meeting": ("meeting", "Meeting", True),
    "DemoNote": ("demo-note", "Demo note", False),
}
