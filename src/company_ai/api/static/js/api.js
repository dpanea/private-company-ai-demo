...
async function mockRequest(path, options) {
  ...
  if (path === "/session" && method === "DELETE") {
    ...
    for (const accountId of Object.keys(mockArtifacts)) {
      mockArtifacts[accountId] = mockArtifacts[accountId].filter((item) => !String(item.artifact_id).startsWith("demo-note:"));
    }
    return { ok: true };
  }
  ...
  const accountDemoNotes = path.match(/^\/accounts\/([^/]+)\/demo-notes$/);
  if (accountDemoNotes) {
    const accountId = decodeURIComponent(accountDemoNotes[1]);
    if (method === "POST") return addMockNote(accountId, body);
    return mockDemoNotes[accountId] || [];
  }
  ...
  const deleteNote = path.match(/^\/demo-notes\/([^/]+)$/);
  if (deleteNote && method === "DELETE") {
    const noteId = decodeURIComponent(deleteNote[1]);
    for (const accountId of Object.keys(mockDemoNotes)) {
      mockDemoNotes[accountId] = mockDemoNotes[accountId].filter((note) => note.note_id !== noteId);
      mockArtifacts[accountId] = (mockArtifacts[accountId] || []).filter((item) => item.artifact_id !== `demo-note:${noteId}`);
    }
    return null;
  }
...
function addMockNote(accountId, payload) {
  ...
  mockDemoNotes[accountId] = [note, ...(mockDemoNotes[accountId] || [])];
  const newArtifact = artifact(
    ...
    { source_object: "DemoNote", source_record_id: noteId, date: payload.note_date, synthetic: true, demo_note: true },
  );
