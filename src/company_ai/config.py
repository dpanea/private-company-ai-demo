...
def _resolve_database_url() -> str:
    ...
    if url:
        return url
    user = _env_optional("POSTGRES_USER") or "company_ai"
    password = _env_optional("POSTGRES_PASSWORD") or "company_ai"
    database = _env_optional("POSTGRES_DB") or "company_ai"
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        settings = cls(
            ...
            app_title=os.environ.get("APP_TITLE", "The Company Knowledge AI"),
        ...
        logger.debug(
            "settings.loaded database_url_configured=%s llm_key_configured=%s embedding_model=%s llm_model=%s embedding_dimensions=%s reasoning_effort=%s context_token_budget=%s log_level=%s log_color=%s",
    ...
    def require_llm_api_key(self) -> None:
        if not self.llm_api_key:
            ...
            raise RuntimeError("LLM_API_KEY is required. Add it to local .env.")
